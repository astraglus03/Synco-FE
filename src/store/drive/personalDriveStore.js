...
// 개인 드라이브 스토어
export const usePersonalDriveStore = defineStore('personalDrive', () => {
  // 상태
  const items = ref([])
  ...
  const loadItems = async (driveChannelSeq, parentId = null) => {
    ...
    try {
      ...
      console.error('개인 드라이브 목록 로드 실패:', err)
    } finally {
  ...
  const uploadFiles = async (files, parentId = null) => {
    ...
    try {
      ...
      console.error('개인 드라이브 파일 업로드 실패:', err)
  ...
  const createFolder = async (name, parentId = null) => {
    ...
    try {
      ...
      console.error('개인 드라이브 폴더 생성 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
  ...
  const createSharedDocument = async (name, parentId = null, isLocked = false) => {
    ...
    try {
      ...
      console.error('개인 드라이브 공유문서 생성 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
  ...
  const deleteItem = async (itemId, itemType) => {
    ...
    try {
      const result = await personalDriveApi.deleteItem(itemId, itemType, currentDriveChannelSeq.value)
      ...
      console.error('개인 드라이브 아이템 삭제 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
  ...
  const renameFolder = async (folderId, newName) => {
    ...
    try {
      const result = await personalDriveApi.renameFolder(folderId, newName, currentDriveChannelSeq.value)
      ...
      console.error('개인 드라이브 폴더 이름 변경 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
  ...
  const renameDocument = async (documentId, newName) => {
    ...
    try {
      const result = await personalDriveApi.renameDocument(documentId, newName, currentDriveChannelSeq.value)
      ...
      console.error('개인 드라이브 문서 이름 변경 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
  ...
  const downloadFile = async (item) => {
    try {
      const result = await personalDriveApi.downloadFile(item.id, currentDriveChannelSeq.value)
      ...
      console.error('개인 드라이브 파일 다운로드 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const downloadDocument = async (documentId) => {
    try {
      ...
      console.error('개인 드라이브 문서 다운로드 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const loadDocumentContent = async (documentId) => {
    try {
      ...
      console.error('개인 드라이브 문서 내용 조회 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const saveDocumentContent = async (documentId, content) => {
    try {
      const result = await personalDriveApi.saveDocumentContent(documentId, currentDriveChannelSeq.value, content, true)
      ...
      console.error('개인 드라이브 문서 저장 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const toggleDocumentLock = async (documentId) => {
    try {
      const result = await personalDriveApi.toggleDocumentLock(documentId, currentDriveChannelSeq.value, true)
      ...
      console.error('개인 드라이브 문서 잠금 상태 변경 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const moveItem = async (itemId, itemType, newParentId) => {
    try {
      const result = await personalDriveApi.moveItem(itemId, itemType, newParentId, currentDriveChannelSeq.value, true)
      ...
      console.error('개인 드라이브 아이템 이동 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const reorderFolder = async (folderId, newOrder) => {
    try {
      const result = await personalDriveApi.reorderFolder(folderId, newOrder, currentDriveChannelSeq.value, true)
      ...
      console.error('개인 드라이브 폴더 순서 변경 실패:', err)
      return { success: false, error: error.value }
    }
  }
  ...
  const getAllFolders = async () => {
    ...
    try {
      ...
      if (result.success) {
        // convertFolderTree에서 이미 DriveItem.fromApiFormat으로 변환 완료
      ...
      console.error('개인 드라이브 전체 폴더 조회 실패:', err)
      return { success: false, error: error.value }
    } finally {
      isLoading.value = false
    }
