...
const hasChannelManagePermission = (channelType) => {
  ...
  return (
    currentUserAuthority === "SUPER" ||
    currentUserAuthority === "MANAGER" ||
    currentUserAuthority === "PARTICIPANT"
  );
};
...
const createSchedule = () => {
  ...
  const scheduleChannel = currentChannels.value.find(
    (ch) => ch.id === "schedule"
  );
...
<template>
  ...
  >
    ...
    <div class="channels-section">
      ...
      >
        ...
        <span v-if="!collapsed" class="project-name">{{
          currentWorkspaceData.name
        }}</span>
      </div>

      <div class="section-title">
        <span v-if="!collapsed || workspaceType === 'personal'">{{
  ...
  <v-dialog v-model="showCreateScheduleModal" max-width="400">
    <v-card class="create-schedule-modal">
      ...
      <div class="modal-actions">
        <v-btn variant="text" @click="closeCreateScheduleModal"> 취소 </v-btn>
        <v-btn
          color="primary"
